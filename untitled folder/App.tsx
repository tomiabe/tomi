...
// CRITICAL: Using strictly relative paths to ensure browser compatibility in the preview environment.
...
const Build: React.FC<SectionProps<SiteContent['build']> & { onNavigate: (path: string) => void }> = ({ data, settings, onNavigate }) => (
  <div className="space-y-12 text-lg max-w-3xl">
    ...
    <div className="italic opacity-50 text-sm pt-4"><RichText content={data.footer} accentColor={settings?.accentColor} /></div>
  </div>
);
...
const App: React.FC = () => {
  ...
  const [currentPath, setCurrentPath] = useState(() => (typeof window !== 'undefined' ? (window.location.pathname === '/studio' ? '/studio' : '/') : '/'));
  const handleNavigate = (path: string) => { window.history.pushState({}, '', path); setCurrentPath(path); window.scrollTo({ top: 0, behavior: 'smooth' }); };
  useEffect(() => { const handlePopState = () => setCurrentPath(window.location.pathname === '/studio' ? '/studio' : '/'); window.addEventListener('popstate', handlePopState); return () => window.removeEventListener('popstate', handlePopState); }, []);
  const [theme, setTheme] = useState<'light' | 'dark'>(() => { if (typeof window !== 'undefined') { const savedTheme = localStorage.getItem('site_theme'); if (savedTheme === 'light' || savedTheme === 'dark') return savedTheme; } const hour = new Date().getHours(); return (hour >= 19 || hour < 7) ? 'dark' : 'light'; });
  ...
  useEffect(() => { if (content?.settings?.siteTitle) document.title = currentPath === '/studio' ? "Tomi Abe Studio" : content.settings.siteTitle; }, [content, currentPath]);
  const toggleTheme = () => { const newTheme = theme === 'dark' ? 'light' : 'dark'; setTheme(newTheme); localStorage.setItem('site_theme', newTheme); };
  useEffect(() => document.documentElement.classList.toggle('dark', theme === 'dark'), [theme]);
  const handleColorSelection = (mode: 'dark' | 'light', shade: string) => { if (mode === 'dark') { setUserDarkTheme(shade); localStorage.setItem('user_dark_theme', shade); } else { setUserLightTheme(shade); localStorage.setItem('user_light_theme', shade); } };
  const handleFontSelection = (font: string) => { setUserFont(font); localStorage.setItem('user_font', font); };
  useEffect(() => { const observer = new IntersectionObserver((entries) => entries.forEach((entry) => { if (entry.isIntersecting) setActiveSection(entry.target.id as SectionId); }), { rootMargin: '-20% 0px -60% 0px' }); const sections = document.querySelectorAll('section[id]'); sections.forEach((section) => observer.observe(section)); const handleScroll = () => setShowScrollTop(window.scrollY > 400); window.addEventListener('scroll', handleScroll); return () => { sections.forEach((section) => observer.unobserve(section)); window.removeEventListener('scroll', handleScroll); }; }, []);
  const scrollToSection = (id: string) => { const el = document.getElementById(id); if (el) el.scrollIntoView({ behavior: 'smooth' }); setActiveSection(id as SectionId); };
  ...
  const getThemeClasses = () => { if (theme === 'dark') { switch (activeDarkShade) { case 'grey': return 'bg-[#18181b] text-zinc-200'; case 'navy': return 'bg-[#0f172a] text-slate-200'; case 'maroon': return 'bg-[#2a0a0a] text-rose-100'; default: return 'bg-black text-zinc-100'; } } switch (activeLightShade) { case 'ash': return 'bg-zinc-100 text-zinc-900'; case 'ivory': return 'bg-[#fffbeb] text-stone-900'; case 'sky': return 'bg-sky-50 text-sky-950'; default: return 'bg-white text-zinc-900'; } };
  ...
  return (
    <div className={`min-h-screen font-${fontFamily} transition-colors duration-500 relative ${themeClasses}`}>
         ...
         <div className="space-y-12 pb-10">
            ...
            <div>
               ...
               <div className="grid grid-cols-2 md:grid-cols-3 gap-x-4 gap-y-2">
                  {['sans', 'apple', 'manrope', 'dmsans', 'figtree', 'ibmplex', 'plusjakarta', 'librefranklin', 'publicsans'].map((id) => (
                    <button key={id} onClick={() => handleFontSelection(id)} className={`flex items-center justify-between px-4 py-4 rounded-xl border-2 transition-all ${fontFamily === id ? `border-blue-500 bg-black/5 dark:bg-white/5` : 'border-transparent hover:bg-black/5 dark:hover:bg-white/5'}`}><span className="text-sm sm:text-base font-medium opacity-90">{id.charAt(0).toUpperCase() + id.slice(1)}</span>{fontFamily === id && <Check className="w-4 h-4 text-blue-500" />}</button>
                  ))}
               </div>
            </div>
      ...
      {currentPath === '/studio' ? <StudioPage content={studioContent} settings={content.settings} /> : (
         <div className="max-w-7xl mx-auto px-6 lg:px-8 transition-all duration-300 pt-6 lg:pt-0">
           <div className="lg:flex lg:gap-12">
             <aside className="lg:w-1/3 xl:w-1/4 lg:h-screen lg:sticky lg:top-0 py-6 lg:py-12 px-2 flex flex-col justify-between overflow-y-auto no-scrollbar gap-12">
               <div>
                 <div className="flex items-center justify-between mb-6 lg:mb-8"><Avatar src={content.intro.avatar} linkToHome={content.intro.avatarLinkToHome} /><div className="flex gap-2 lg:hidden"><button onClick={() => setSettingsOpen(true)} className="p-2 rounded-full opacity-60 hover:opacity-100 hover:bg-black/5 dark:hover:bg-white/10 transition-all"><Swatch className="w-5 h-5" /></button><button onClick={toggleTheme} className="p-2 rounded-full opacity-60 hover:opacity-100 hover:bg-black/5 dark:hover:bg-white/10 transition-all">{theme === 'dark' ? <Moon className="w-5 h-5" /> : <Sun className="w-5 h-5" />}</button></div></div>
                 ...
                 <div className="lg:hidden mb-2 space-y-4"><div className="flex gap-4">{content.intro.socials.map((link) => <a key={link.id} href={link.url} target="_blank" rel="noopener noreferrer" className="opacity-50 opacity-100 transition-colors">{getIcon(link.platform)}</a>)}</div><Clock /></div>
                 <nav className="hidden lg:block space-y-1 mt-8">{[ { id: 'who', label: 'Who Am I' }, { id: 'build', label: 'What Do I Build' }, { id: 'learning', label: 'What Am I Learning' }, { id: 'share', label: 'Where Do I Share' }, { id: 'see', label: 'What Do I See' }, { id: 'connect', label: 'Connect' } ].map((item) => { const isActive = activeSection === item.id; return (<button key={item.id} onClick={() => scrollToSection(item.id)} className={`group flex items-center w-full py-2 text-sm font-medium transition-colors ${isActive ? 'opacity-100' : 'opacity-50 hover:opacity-80'}`}><span className="w-8 h-px mr-3 transition-all duration-300" style={{ backgroundColor: isActive ? accentColor : 'currentColor', width: isActive ? '3rem' : '2rem', opacity: isActive ? 1 : 0.3 }}></span>{item.label}</button>); })}</nav>
               </div>
               <div className="hidden lg:block"><div className="mb-6"><Clock /></div><div className="flex gap-3 mb-6 flex-wrap">{content.intro.socials.map((link) => <a key={link.id} href={link.url} target="_blank" rel="noopener noreferrer" className="opacity-50 hover:opacity-100 transition-colors p-1">{getIcon(link.platform)}</a>)}</div><div className="flex items-center justify-between"><div className="flex items-center gap-2"><button onClick={toggleTheme} className="p-1.5 rounded-full opacity-40 hover:opacity-100 hover:bg-black/5 dark:hover:bg-white/10 transition-colors">{theme === 'dark' ? <Moon className="w-4 h-4"/> : <Sun className="w-4 h-4"/>}</button><button onClick={() => setSettingsOpen(true)} className="p-1.5 rounded-full opacity-40 hover:opacity-100 hover:bg-black/5 dark:hover:bg-white/10 transition-colors"><Swatch className="w-4 h-4" /></button></div></div><div className="text-xs opacity-50 mt-6 font-medium">&copy; 2026 {content.intro.name}.</div></div>
             </aside>
             <main className="lg:w-2/3 xl:w-3/4 py-6 lg:py-12 space-y-24 lg:space-y-32">
               ...
               <section id="who" className="scroll-mt-24"><div className="lg:hidden mb-6 pb-2 border-b border-black/10 dark:border-white/10 text-xs font-bold uppercase tracking-widest opacity-50">Who Am I</div><Who data={content.who} settings={content.settings} /></section>
               <section id="build" className="scroll-mt-24"><div className="lg:hidden mb-6 pb-2 border-b border-black/10 dark:border-white/10 text-xs font-bold uppercase tracking-widest opacity-50">What Do I Build</div><Build data={content.build} settings={content.settings} onNavigate={handleNavigate} /></section>
               <section id="learning" className="scroll-mt-24"><div className="lg:hidden mb-6 pb-2 border-b border-black/10 dark:border-white/10 text-xs font-bold uppercase tracking-widest opacity-50">What Am I Learning</div><Learning data={content.learning} settings={content.settings} /></section>
               <section id="share" className="scroll-mt-24"><div className="lg:hidden mb-6 pb-2 border-b border-black/10 dark:border-white/10 text-xs font-bold uppercase tracking-widest opacity-50">Where Do I Share</div><Share data={content.share} settings={content.settings} /></section>
               <section id="see" className="scroll-mt-24"><div className="lg:hidden mb-6 pb-2 border-b border-black/10 dark:border-white/10 text-xs font-bold uppercase tracking-widest opacity-50">What Do I See</div><See data={content.see} settings={content.settings} /></section>
               <section id="connect" className="scroll-mt-24 pb-0 lg:pb-12"><div className="lg:hidden mb-6 pb-2 border-b border-black/10 dark:border-white/10 text-xs font-bold uppercase tracking-widest opacity-50">Connect</div><Connect data={content.connect} settings={content.settings} /></section>
