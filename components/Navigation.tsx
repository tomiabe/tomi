    ...
    <>
      <nav 
        className={`fixed top-0 left-0 right-0 z-40 transition-all duration-300 ${
          scrolled ? 'bg-white/80 dark:bg-black/80 backdrop-blur-md border-b border-zinc-200/50 dark:border-zinc-800/50 py-3' : 'bg-transparent py-5'
        }`}
      >
        <div className="max-w-7xl mx-auto px-6 lg:px-8 flex items-center justify-between">
          
          {/* Logo Area */}
          <div className="flex items-center gap-4 cursor-pointer" onClick={() => onNavigate(isStudio ? '/' : '/studio')}>
             {isStudio ? (
          ...
          <div className="flex items-center gap-3">
             {/* Switcher Link */}
             <button 
               onClick={() => onNavigate(isStudio ? '/' : '/studio')}
               className="text-xs font-medium uppercase tracking-wider text-zinc-500 hover:text-zinc-900 dark:hover:text-white transition-colors mr-2 hidden sm:block"
             >
               {isStudio ? 'Personal Site →' : 'Studio →'}
             </button>
