...
function moveDemoCursor(selector) {
  if (!selector || typeof selector !== 'string') return Promise.resolve();
  let target = null;
  try {
...
function setupCaseStudyDemo() {
  ...
  document.addEventListener('pointerdown', event => {
    if ((demoRunning || demoRepeatTimer) && !event.target.closest('.case-demo-cursor')) stopDemo(true);
  }, true);
  document.addEventListener('focusin', event => {
    if ((demoRunning || demoRepeatTimer) && !event.target.closest('.case-demo-cursor')) stopDemo(true);
  }, true);
  window.addEventListener('message', event => {
    if (event.data?.type === 'eze-demo:start') startDemo();
  });
  if (window.parent !== window) {
    try {
      window.parent.postMessage({ type: 'eze-demo:ready', demo: demoMode }, '*');
    } catch (e) {}
    window.setTimeout(() => {
      if (!demoRunning && !demoPausedByUser) startDemo();
    }, 1200);
  } else {
